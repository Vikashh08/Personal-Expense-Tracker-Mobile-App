...
import { getIncomesApi, deleteIncomeApi } from '../../src/features/income/api/incomeApi';
...
export default function IncomeHistoryScreen() {
  ...
  const handleDelete = (id: string) => {
    Alert.alert('Delete Income', 'Are you sure you want to delete this transaction?', [
  ...
  if (isError) {
    return (
      <View className="flex-1 justify-center items-center bg-slate-50">
        <Text className="text-red-500 mb-4">Failed to load income history</Text>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View className="items-center justify-center mt-12 p-8">
            ...
            <Text className="text-slate-500 text-center">No income transactions found.</Text>
          </View>
        }
        renderItem={({ item }) => (
          <View className="bg-white p-4 rounded-2xl mb-3 shadow-sm shadow-slate-100 flex-row justify-between items-center">
            <View className="flex-row items-center flex-1">
              ...
              <View className="flex-1">
                <Text className="text-slate-800 font-semibold text-lg">{item.category?.name || 'Income'}</Text>
