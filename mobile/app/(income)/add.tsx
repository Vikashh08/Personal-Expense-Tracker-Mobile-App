...
import { addIncomeApi } from '../../src/features/income/api/incomeApi';
  ...
  return (
    <ScrollView className="flex-1 bg-slate-50 px-6 pt-6">
      <View className="mb-6">
        ...
        <TextInput
          className="bg-white border border-slate-200 rounded-xl px-4 py-3 text-2xl font-bold text-emerald-600"
          placeholder="0.00"
      ...
      <View className="mb-6">
        <Text className="text-slate-700 font-medium mb-2">Account</Text>
        <View className="flex-row flex-wrap">
          {accounts?.map((acc: any) => (
            <TouchableOpacity
              key={acc.id}
              onPress={() => setSelectedAccountId(acc.id)}
              className={`mr-2 mb-2 px-4 py-2 rounded-xl border ${selectedAccountId === acc.id || (!selectedAccountId && accounts[0].id === acc.id) ? 'bg-indigo-100 border-indigo-600' : 'bg-white border-slate-200'}`}
            >
              <Text className={selectedAccountId === acc.id || (!selectedAccountId && accounts[0].id === acc.id) ? 'text-indigo-600 font-bold' : 'text-slate-600'}>{acc.name}</Text>
            </TouchableOpacity>
          ))}
        </View>
      </View>
      ...
      <TouchableOpacity
        className="w-full bg-indigo-600 rounded-xl py-4 items-center shadow-lg shadow-indigo-300 mt-4"
        onPress={handleSave}
